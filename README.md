...
[デザインハーネス](https://design-harness.com/)の方法論を、業務画面向けの既存デザインシステムからスライド領域へ移植する。同じお題を「設計契約なし」と「契約あり」で AI に生成させ、その差と修正過程を比較する。
...
**正本に置いた値・一覧を、他の文書へ書き写さない。** 書き写されたことは `pnpm canonical:check` が機械で検出する（走査する場所と検出する形には限りがあり、範囲と塞げていない形は [DR-0046](./docs/decisions/0046-prose-checked-for-canonical-duplication.md) にある）。書かざるを得ない箇所は `scripts/canonical-duplication-allowlist.json` へ理由つきで登録する。理由の無い例外と、使われなくなった例外は検査が落とす。根拠は [DR-0046](./docs/decisions/0046-prose-checked-for-canonical-duplication.md)。
