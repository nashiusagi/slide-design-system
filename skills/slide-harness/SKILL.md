...
`DESIGN.md` / `design/` の設計契約をそのまま実装の拠り所として使う。**この Skill の中に契約の中身（色・数値・layout や component の説明文）を複製しない。** 契約は常に `DESIGN.md` / `design/` を正本として参照する（[DR-0013](../../docs/decisions/0013-agent-skill-and-resolver.md)）。
...
3. `HARNESS_RESOLVED.json` がカレントディレクトリにあれば読む。無ければ、このタスクが必要とする deck / layout / component を manifest として用意し、解決する。

   `node scripts/resolve-design-contract.mjs <manifest.json>`

   manifest の形は `{ "decks": [...], "layouts": [...], "components": [...] }`。deck を指定すると、その deck が使う layout と component は自動で解決結果へ含まれる。少なくとも一つの参照が要る。実行すると `HARNESS_RESOLVED.json` がカレントディレクトリへ書き出される。
