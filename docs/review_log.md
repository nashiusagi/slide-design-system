...
最終更新: 2026-09-21（PR #53 レビュー 3周目・打ち切り）
...
| `contract/design-data-duplicated` | デザインの値が正本以外へ複製された | 11 | PR #53 | 未着手 |
| `writing/inaccurate-rationale` | 説明文が書いている理由付けが、実装の挙動と食い違う | 12 | PR #53 | 未着手 |
...
| `inspection/rule-coverage-partial` | 判定対象の列挙に穴があり、基準を割った組み合わせを見逃す | 8 | PR #53 | 一部（[DR-0044](./decisions/0044-bypass-fixtures-required.md) / #43）。`enumeration-tail` の軸で事例を要求するが、網羅性の証明ではない |
| `phase/completion-criterion-not-verified-e2e` | Issue の完了条件が、単体テストのみで統合パスを通さず検証されている | 6 | PR #53 | 未着手 |
...
|---|---|---|---|---|---|
| `contract/design-data-duplicated` | 設計契約 | デザインの値が正本以外へ複製された | 11 | PR #13 | PR #53 |
...
| `decisions/undocumented-decision` | 決定記録 | DR に無い判断が、PR説明やIssueにだけ書かれている | 13 | PR #13 | PR #53 |
...
| `decisions/dr-restates-canonical-value` | 決定記録 | DR が正本の値を本文に書き写した | 7 | PR #17 | PR #52 |
...
| `inspection/rule-coverage-partial` | 検査 | 判定対象の列挙に穴があり、基準を割った組み合わせを見逃す | 8 | PR #20 | PR #53 |
...
| `writing/inaccurate-rationale` | 日本語 | 説明文が書いている理由付けが、実装の挙動と食い違う | 12 | PR #23 | PR #53 |
...
| `phase/completion-criterion-not-verified-e2e` | フェーズ | Issue の完了条件が、単体テストのみで統合パスを通さず検証されている | 6 | PR #25 | PR #53 |
...
| `contract/allowlist-key-tracks-canonical-values` | 設計契約 | 例外リストの照合キーが、正本の値どうしの偶然の衝突に依存する | 1 | PR #52 | PR #52 |
