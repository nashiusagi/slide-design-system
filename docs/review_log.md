...
| `inspection/rule-has-bypass` | 検査ルールに抜け道があり、書き方を変えると素通りする | 8 | PR #25 | 未着手 |
...
| `contract/design-data-duplicated` | デザインの値が正本以外へ複製された | 3 | PR #24 | 未着手 |
| `writing/inaccurate-rationale` | 説明文が書いている理由付けが、実装の挙動と食い違う | 3 | PR #25 | 未着手 |
| `inspection/rule-coverage-partial` | 判定対象の列挙に穴があり、基準を割った組み合わせを見逃す | 3 | PR #25 | 未着手 |

## 全カテゴリ

| カテゴリID | 観点 | 要約 | 累計 | 初出 | 最終指摘 |
|---|---|---|---|---|---|
| `contract/design-data-duplicated` | 設計契約 | デザインの値が正本以外へ複製された | 3 | PR #13 | PR #24 |
...
| `decisions/undocumented-decision` | 決定記録 | DR に無い判断が、PR説明やIssueにだけ書かれている | 7 | PR #13 | PR #24 |
...
| `inspection/rule-coverage-partial` | 検査 | 判定対象の列挙に穴があり、基準を割った組み合わせを見逃す | 3 | PR #20 | PR #25 |
...
| `inspection/rule-has-bypass` | 検査 | 検査ルールに抜け道があり、書き方を変えると素通りする | 8 | PR #13 | PR #25 |
...
| `writing/inaccurate-rationale` | 日本語 | 説明文が書いている理由付けが、実装の挙動と食い違う | 3 | PR #23 | PR #25 |
...
| `phase/completion-criterion-not-verified-e2e` | フェーズ | Issue の完了条件が、単体テストのみで統合パスを通さず検証されている | 1 | PR #25 | PR #25 |
