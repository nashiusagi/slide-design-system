...
| `writing/term-inconsistency` | 同じものが複数の呼び名を持ち、外延も揺れる | 8 | PR #26 | 未着手 |
...
| `decisions/undocumented-decision` | DR に無い判断が、PR説明やIssueにだけ書かれている | 8 | PR #26 | 未着手 |
...
| `inspection/rule-has-bypass` | 検査ルールに抜け道があり、書き方を変えると素通りする | 9 | PR #26 | 未着手 |
...
| `contract/design-data-duplicated` | デザインの値が正本以外へ複製された | 4 | PR #26 | 未着手 |
| `writing/inaccurate-rationale` | 説明文が書いている理由付けが、実装の挙動と食い違う | 4 | PR #26 | 未着手 |
| `inspection/rule-coverage-partial` | 判定対象の列挙に穴があり、基準を割った組み合わせを見逃す | 4 | PR #26 | 未着手 |

## 全カテゴリ

| カテゴリID | 観点 | 要約 | 累計 | 初出 | 最終指摘 |
|---|---|---|---|---|---|
| `contract/design-data-duplicated` | 設計契約 | デザインの値が正本以外へ複製された | 4 | PR #13 | PR #26 |
...
| `writing/term-inconsistency` | 日本語 | 同じものが複数の呼び名を持ち、外延も揺れる | 8 | PR #13 | PR #26 |
...
| `decisions/undocumented-decision` | 決定記録 | DR に無い判断が、PR説明やIssueにだけ書かれている | 8 | PR #13 | PR #26 |
...
| `decisions/consequence-not-followed` | 決定記録 | 既存 DR の帰結が、それを通る手順に反映されていない | 4 | PR #17 | PR #25 |
...
| `inspection/rule-coverage-partial` | 検査 | 判定対象の列挙に穴があり、基準を割った組み合わせを見逃す | 4 | PR #20 | PR #26 |
...
| `inspection/rule-has-bypass` | 検査 | 検査ルールに抜け道があり、書き方を変えると素通りする | 9 | PR #13 | PR #26 |
...
| `writing/inaccurate-rationale` | 日本語 | 説明文が書いている理由付けが、実装の挙動と食い違う | 4 | PR #23 | PR #26 |
