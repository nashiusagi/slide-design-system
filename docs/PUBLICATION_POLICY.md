...
- **`root` / `admin` / `node` のような汎用アカウント名、および4文字未満の短い名前は、単独文字列としての検出対象から意図的に外している。** コンテナ・CI ではこうした名前で実行することが多く、含めると `id="root"` のような無関係なボイラープレートやソースコードの短い識別子まで誤検知するため（一覧は `scripts/audit-public-data.mjs` の `GENERIC_USERNAMES` を正本とする）。この一覧は既知のものへ追従する運用であり、網羅を保証しない
