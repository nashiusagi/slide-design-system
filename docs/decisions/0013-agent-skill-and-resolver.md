...
- SKILL.md に設計データが複製されていないことを検査する仕組みを持つ。対象は色（`design/tokens.json` の color）、layout / component の役割・使用基準の文章、rule の description、および `DESIGN.md` の記述（見出し・箇条書き記号を除いた20文字以上の行）。小さい数値と20文字未満の短い行は、散文の一般的な表現と衝突しやすく誤検知になるため、この検査では対象にしない。値と一覧の複製は、判定の形を絞ったうえで `scripts/check-canonical-duplication.mjs` が別に見る（[DR-0046](./0046-prose-checked-for-canonical-duplication.md)）
