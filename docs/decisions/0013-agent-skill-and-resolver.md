...
- SKILL.md に設計データが複製されていないことを検査する仕組みを持つ。対象は色（`design/tokens.json` の color）、layout / component の役割・使用基準の文章、rule の description、および `DESIGN.md` の記述。`space` / `max` のような小さい数値は一般的な語（「3」等）と衝突しやすく誤検知になるため対象にしない
