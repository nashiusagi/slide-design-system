...
| [0056](./0056-slide-content-scored-against-contract.md) | スライドが keyMessage を伝えているかを、レイアウトの役割ごとの軸で判定する |
...
既存の決定を覆す場合は、新しい DR を立てて旧 DR の状態を「置き換え済み」に変える。旧 DR は削除しない。**決定の一部だけを覆す場合は**、旧 DR の状態を「承認済み（`<置き換わった範囲>` は DR-NNNN が置き換えた）」とし、該当箇所と帰結にも同じことを書く。全面の置き換えと区別が付かないと、状態欄から有効・無効を読めなくなる（[DR-0052](./0052-review-loop-third-exit.md) が最初の例）。
