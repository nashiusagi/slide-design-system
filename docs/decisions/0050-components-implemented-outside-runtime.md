...
- **状態**: 承認済み
- **日付**: 2026-09-21
- **関連**: [DR-0018](./0018-plain-css-with-tokens.md), [DR-0021](./0021-starter-contains-runtime-only.md), [DR-0030](./0030-slide-class-derived-from-layout.md), [DR-0035](./0035-layout-component-contract-shape.md), [DR-0038](./0038-defer-measure-in-check.md), [DR-0042](./0042-design-catalog-as-separate-build-entry.md), [DR-0049](./0049-catalog-detects-implementation-by-registry.md)
...
形のほうにも既存の前提がある。`component-approved`（[DR-0011](./0011-lint-and-measure.md)）は、契約名を PascalCase にした JSX 要素が `allowedIn` どおりの layout の下で使われているかを見る検査で、正規の実装を import して使う前提で書かれている。
...
契約名の定義をどこにも作れないままでは、import して使う相手が存在しない。どのファイルが正規の実装かはルールオプション（`implementsContracts`）で外から与え、`eslint.config.js` が `src/components/` にだけ付ける。`allowedIn` の判定は外さない。
...
- 部品を1つ増やすときは、契約・実装・同名のクラス・カタログの登録表の4箇所が揃って初めて画面に出る。このうちクラスの欠落は `pnpm design:check` が、登録表の欠落は誰も捕まえない（[DR-0049](./0049-catalog-detects-implementation-by-registry.md) の帰結）
...
- `implementsContracts` を付ける場所は `eslint.config.js` だけに保つ。`src` は抑止コメントを無効にしてある（`noInlineConfig`）ので、ファイル側から自分でこの例外を名乗ることはできない
