...
deck 契約は Phase 1 では lint（`deck-conformance`）の対象であり、Phase 2 では**人間が書く場所**になる。つまり両フェーズの接合部にあたる。参考元の `examples` は JSON だったが、あれは AI しか書かないから成立していた。
...
検証もツールも最も素直で、参考元のスクリプトをそのまま使える。
