...
カタログが用意するのは、キャンバス寸法（`design/tokens.json` の `canvas` を `var(--dh-canvas-*)` で当てる）と `box-sizing: border-box` の2つだけとする。これは `.slide` が実行時に与えているものと同じで、契約ではなく機構の側に属する。
...
- **0件を例外にする。** glob のパスがずれても、ページは「契約が0件」の姿で何事もなく描かれ、件数を突き合わせるテストも `0 === 0` で通る（[DR-0043](./0043-catalog-reads-generated-theme-css.md) 決定3 と同じ形の素通り）
...
**却下理由**: lint をすり抜けるだけで、[DR-0042](./0042-design-catalog-as-separate-build-entry.md) が引いた境界を越えることに変わりはない。`.slide-deck` / `.slide-canvas` という、カタログが使わない規則も一緒に入る。写すのが `box-sizing` の1行である以上、境界を越える側の代償のほうが大きい。
