...
describe('findStructureDuplications', () => {
  ...
  /*
   * 表は区切り行の直前の見出し行から始まる。その手前にパイプを含む地の文が続いていても、
   * 地の文は表のブロックに入らない。ここが緩むと、表と地の文が1つのブロックに溶けて、
   * 表に無い言及まで一覧の複製として報告される。
   */
  it('表の手前にある地の文は、表のブロックに含めない', () => {
    const outside = `今日は \`${layouts[0].name}\` を試した。手順は 準備 | 実行 の順で進める。`
    const table = ['name | 役割', '--- | ---', `\`${layouts[1].name}\` | 説明`]

    // 表の中にある項目は1件だけなので、地の文の項目と繋がらない限り報告は出ない。
    expect(structureIn([outside, ...table].join('\n'))).toEqual([])

    // 同じ表に項目が2件並べば、地の文が手前にあっても報告は出る。
    expect(
      structureIn([outside, ...table, `\`${layouts[2].name}\` | 説明`].join('\n')).length,
    ).toBeGreaterThan(0)
  })
