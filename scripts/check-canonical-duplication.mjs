 ...
 * 見出し行の次の行に区切り行（`--- | ---`）を必ず伴うので、パイプを持つ行が続く範囲から
 * 区切り行を探し、その直前の見出し行から範囲の終わりまでを表として扱う。区切り行を
 * 要求しないと、「前段 | 後段 の順で進む」のような地の文が表の行になり、離れた単発の
 * 言及どうしが1つのブロックへ繋がって誤検出になる。範囲の先頭を見出し行と決め打つと、
 * 見出しの直前にパイプを含む地の文が続いているだけで判定が1行ずれ、その表を丸ごと
 * 見逃す。
 *
 * @param {string[]} lines
 * @returns {boolean[]}
