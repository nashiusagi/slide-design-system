...
| `check-canonical-duplication.mjs` | 正本の値・一覧が、正本を参照できない文書（README / DR / Skill / カタログのコード）へ書き写されていないかを検査する（DR-0046）。例外は `canonical-duplication-allowlist.json` に理由つきで登録する |
