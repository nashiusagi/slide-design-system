 ...
 *
 * **CommonMark のフェンス規則を模す。** この検査が見るのは規則の4つの軸だけだ——
 * インデント（記号の前は3字まで）、記号の種類（`` ` `` か `~`）、長さ（3個以上）、
 * そして閉じの有無。フェンスとして開くのは3字までの字下げで3個以上の記号が並んだ行、
 * 閉じるのは同じ記号が開いた長さ以上続き、その後ろが空白だけの行に限る。
 *
 * **この4軸は、1つずつ穴として見つかったものだ**（PR #61 の1〜4周目の blocker）。
 ...
 * これを使えるようにしておく。
 *
 * **模していない規則もある。** インデントされたコードブロック（4字下げの行）と、
 * 箇条書きの中の相対インデントは扱わない。だから文書全体から見て4字以上下がった
 * 位置にあるフェンス——箇条書きの深い入れ子の中など——はフェンスと見なされず、
 * 中身が地の文として検査される。**倒れる先が偽陽性であって偽陰性ではないことが、
 * この近似を採る根拠だ。** 中の書式サンプルが落ちれば `pnpm check` が赤くなり、
 * 人が気づく。逆に倒すと、黙って緑になる。
 *
 * @param {string} source
