...
/**
 * **フェンスで囲まれた**コードブロックの中身を空行へ置き換える（インデントされた
 * コードブロックは別物で、扱わない——後述）。書式サンプル（`docs/decisions/README.md`
 ...
 *
 * **CommonMark のフェンス規則を模す。** この検査が見るのは規則の5つの軸だけだ——
 * インデント（記号の前は3字まで）、記号の種類（`` ` `` か `~`）、長さ（3個以上）、
 * 情報文字列（バッククォートのフェンスには `` ` `` を置けない）、そして閉じの有無。
 * フェンスとして開くのは3字までの字下げで3個以上の記号が並んだ行、閉じるのは同じ
 * 記号が開いた長さ以上続き、その後ろが空白だけの行に限る。
 *
 * **この5軸は、1つずつ穴として見つかったものだ**（PR #61 の1〜5周目の blocker）。
 * どれを緩めても、フェンスの中と外の境界がずれて検査が黙って効かなくなる。
 *
 * - 閉じの有無を見ない → 閉じ忘れたファイルの残りが丸ごと無検査になる（1周目）
 * - 記号の種類を見ない → ``` の中の `~~~` や ```js で閉じたことになる（2周目）
 * - 長さを記録しない → 4個で開いても、正規表現のキャプチャが3文字固定なら3個として
 *   しか覚えず、中の3個の行で閉じたことになる。逆に開きと閉じの長さの一致を求めると、
 *   開き3・閉じ4 という CommonMark では正しい閉じ方を落とす（3周目）
 * - インデントの上限を見ない → 4字以上下げた行は CommonMark ではフェンスではないのに
 *   開きと見なし、次の本物のフェンスまでの本文を無警告で捨てる（4周目）
 * - 情報文字列を見ない → `` ``` `x` のように書く `` のような行は、バッククォートを
 *   含むので CommonMark ではフェンスではない。開きと見なすと、後方にある無関係な
 *   フェンスの**開き行**を閉じと誤認し、間の本文を無警告で捨てる（5周目）
 *
 * 4個以上のフェンスは、コード例を入れ子で見せるときの標準の書き方だ。書式ファイルが
 * これを使えるようにしておく。
 *
 * **模していない規則もある**（[DR-0054](../docs/decisions/0054-decision-reference-checked-by-machine.md)
 * 決定3）。インデントされたコードブロック（4字下げの行）と、箇条書きの中の相対
 * インデントは扱わない。だから文書全体から見て4字以上下がった位置にあるフェンス——
 * 箇条書きの深い入れ子の中など——はフェンスと見なされず、中身が地の文として検査
 * される。**倒れる先が偽陽性であって偽陰性ではないことが、この近似を採る根拠だ。**
 * 中の書式サンプルが落ちれば `pnpm check` が赤くなり、人が気づく。逆に倒すと、
 * 黙って緑になる。
 *
 * **5つの軸を1周ずつ見つけてきたこと自体が、この進み方の限界を示している。**
 * 規則の全体を先に読まずに指摘のたびに軸を足したので、4周目の時点では軸の総数を
 * 4つと見積もっていた。自前で持ち続けるか既存のパーサへ委ねるかは #63 が引き取る。
 *
 * @param {string} source
  ...
  const stripped = lines
    .map((line, index) => {
      if (fence === undefined) {
        ...
        // バッククォートのフェンスは、情報文字列にバッククォートを置けない。
        // 置いてあればフェンスではなく地の文だ（`~~~` にこの制約は無い）。
        const openable = opening && !(opening[1][0] === '`' && opening[2].includes('`'))
